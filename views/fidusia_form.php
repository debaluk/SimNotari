...
where  tb_fidusia.id_user=tb_user.id_user and tb_fidusia.id_fidusia='$_GET[id]'");
$hasildata = mysqli_fetch_array($data);

if ($_GET[id])
{
	$nourut = $hasildata['noaktajamfud'];
	$tgl= $hasildata['tglakta'];
}
else
{
	$tgl=date("Y-m-d");
	$bulanpilihan =date("Y-m");
...
<link rel="stylesheet" href="https//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
    <div class="modal-dialog" id="modal" style="width:50%";>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title" id="modalAddBrandLabel">Data Akta Fidusia</h4>
            </div>
            <form id="simpanvac" name="simpanvac"  action="" method="post" data-toggle="validator" role="form" class="form-horizontal">
				<input type="hidden" placeholder="" class="form-control" name="id" value="<?php echo $hasildata['id_fidusia'];?>" readonly>
            	<div class="modal-body"> 
					...
					<div class="form-group">
						...
						<label class="col-sm-2 required">NIK</label>
						<div class="col-sm-4">
							<select name="nik" id="nik" class="form-control" required>
								<option value="<?php echo $hasildata[nik];?>"><?php echo $hasildata[nik];?></option>
								<?php
								$dataklien=mysqli_query($con,"select * from tb_klien");
								while ($listklien=mysqli_fetch_array($dataklien))
								{
								?>
								<option value="<?php echo $listklien[nik];?>"><?php echo $listklien[nik];?></option>
								<?php } ?>
							</select>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="text" placeholder="" class="form-control" name="jnsakta" value="Jaminan Fidusia" readonly required>
						</div>
							...
							<table class="table" id="table-product">
								<tbody>
									<tr>
										...
										<td>
											<select name="nobpkb[]" id="nobpkb[]" class="form-control" required>
												<option value="<?php echo $hasildata[id_bpkb];?>"><?php echo $hasildata[nobpkb];?></option>
												<?php
												$databpkb=mysqli_query($con,"select * from tb_bpkb");
												while ($listbpkb=mysqli_fetch_array($databpkb))
												{
												?>
												<option value="<?php echo $listbpkb[id_bpkb];?>"><?php echo $listbpkb[nobpkb];?> | <?php echo $listbpkb[kat_object];?> | <?php echo $listbpkb[merek];?> </option>
												<?php } ?>
											</select>
										</td>
		...
		$('#table-product').find('tbody').append(`
			<tr>
				...
				<td>
					...
					<select name="nobpkb[]" id="nobpkb[]" class="form-control" required>
						<option value="<?php echo $hasildata[nobpkb];?>"><?php echo $hasildata[nobpkb];?></option>
						<?php
						$databpkb=mysqli_query($con,"select * from tb_bpkb");
						while ($listbpkb=mysqli_fetch_array($databpkb))
						{
